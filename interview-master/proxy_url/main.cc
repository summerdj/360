...
void test_ProxUrlExtractor_Extract2()
{
    ...
    for (size_t i = 0; i < H_ARRAY_SIZE(test_data); i++)
    {
 	std::string tmp = ProxyURLExtractor::Extract(keys, test_data[i][0]);
	//cout<< "result is : " <<tmp <<endl;
        if (tmp != test_data[i][1]) {
            fprintf(stderr, "test failed [%s]\n", test_data[i][0].data());
            all_test_ok = false;
 //           assert(false);
        }
...
{
    std::string m = "http://www.microsofttranslator.com/bv.aspx?from=&to=zh-chs&a=http://hnujug.com/";
